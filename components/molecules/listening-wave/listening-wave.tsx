...
function softNoise(angle: number, t: number) {
    return (
        Math.sin(angle + t) +
        Math.sin(angle * 2 + t * 0.5) +
        Math.sin(angle * 0.5 + t * 1.5)
    ) / 3;
}
...
export function VoiceBlobSmoothCircle({ isSpeaking = false, aiVolume }: { isSpeaking?: boolean, aiVolume?: number }) {
    ...
    return (
        <div className="relative flex items-center justify-center w-64 h-64">
            ...
            <div className="absolute z-10 w-40 h-40  rounded-full shadow-lg transition-opacity duration-300 ease-in-out flex items-center justify-center">
                <div
                    className="w-16 h-16 items-center justify-center flex bg-white rounded-full shadow-lg z-10"
                >
