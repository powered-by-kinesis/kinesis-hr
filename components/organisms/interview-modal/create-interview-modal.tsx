...
interface CreateInterviewModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: CreateInterviewRequestDTO) => Promise<any>; // Updated return type
}
...
export function CreateInterviewModal({
    ...
    React.useEffect(() => {
        ...
        if (isOpen && currentStep === 3) { // Changed from 2 to 3
            fetchApplicants();
        }
    }, [isOpen, currentStep]);
                        ...
                        {currentStep === 2 && (
                            <div className="space-y-4">
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        const currentQuestions = form.getValues('customQuestionList') || [];
                                        form.setValue('customQuestionList', [...currentQuestions, { question: '', time: 0 }]);
                                    }}
                                >
                                    Add Question
                                </Button>
                        ...
                        {currentStep === 4 && (
                            <div className="space-y-4">
                                ...
                                {form.watch('jobPostId') && (
                                    <p>Job Post: {jobPosts.find(job => job.id === form.watch('jobPostId'))?.title}</p>
                                )}
                                <p>Selected Applicants:</p>
                                <ul>
                                    {selectedApplicantIds.map(id => {
                                        const applicant = applicants.find(app => app.id === id);
                                        return <li key={id}>- {applicant?.fullName} ({applicant?.email})</li>;
                                    })}
                                </ul>
                                <p>Skills:</p>
                                <ul>
                                ...
                                <ul>
                                    {(form.watch('customQuestionList') || []).map((question, index) => (
                                        <li key={index}>- {question.question} ({question.time} min)</li>
                                    ))}
                                </ul>
                            </div>
                        )}
